import { updatePlatformSettingsAction, updateUserPreferencesAction } from "@/actions/learning-core";
...
export default async function SettingsPage() {
  ...
  return <div className="grid gap-6"><PageHeader title="الإعدادات" description="اضبط تجربتك الشخصية، وإعدادات المنصة العامة إذا كنت مديرًا."/><div className="grid items-start gap-6 xl:grid-cols-2">
    <Card><CardTitle>تفضيلات العرض</CardTitle><CardDescription>تُطبّق على حسابك فقط.</CardDescription><ActionForm action={updateUserPreferencesAction} className="mt-5 grid gap-5"><Field label="السمة"><Select name="theme" defaultValue={preferences.theme}><option value="system">حسب الجهاز</option><option value="light">فاتح</option><option value="dark">داكن</option></Select></Field><label className="flex min-h-14 cursor-pointer items-center justify-between gap-4 rounded-2xl border border-[var(--border)] p-4"><span><strong className="block">تقليل الحركة</strong><small className="text-[var(--muted)]">يوقف الحركات غير الضرورية.</small></span><input type="checkbox" name="reducedMotion" value="true" defaultChecked={preferences.reducedMotion} className="size-5 accent-[#0f8b8d]"/></label><Button>حفظ تفضيلاتي</Button></ActionForm></Card>
