...
import { FileText, UploadCloud, Video, XCircle } from "lucide-react";
import { Button } from "@/components/ui/button";
import { createClient } from "@/lib/supabase/client";

export function UploadPanel({ lessonId, lessonPartId, kind }: { lessonId: string; lessonPartId?: string; kind: "video" | "handout" }) {
      ...
      if (setup.mode === "demo") {
        ...
        await new Promise<void>((resolve, reject) => {
          const uploader = new tus.Upload(file, {
            endpoint: `${setup.storageUrl}/storage/v1/upload/resumable`,
            retryDelays: [0, 3000, 5000, 10000, 20000],
            headers: { authorization: `Bearer ${session.access_token}`, "x-upsert": "false" },
            uploadDataDuringCreation: true,
            removeFingerprintOnSuccess: true,
            metadata: { bucketName: setup.bucket!, objectName: setup.objectPath!, contentType: file.type, cacheControl: "3600" },
  ...
  return <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5"><input ref={input} type="file" hidden accept={kind === "video" ? "video/mp4,video/webm" : "application/pdf"} onChange={(event) => { const file=event.target.files?.[0]; if(file) void upload(file); }}/><div className="flex flex-wrap items-center gap-4"><div className="grid size-12 place-items-center rounded-xl bg-white text-[#1479b8] shadow-sm"><Icon className="size-6"/></div><div className="min-w-0 flex-1"><p className="font-black">{kind === "video" ? "فيديو الدرس" : "ملزمة PDF"}</p><p className="mt-1 text-xs text-slate-500">رفع حقيقي مع حالة واضحة، والاستبدال لا يحذف الملف القديم قبل نجاح الجديد.</p>{progress !== null && <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-[#1479b8] transition-all" style={{ width: `${progress}%` }}/></div>}{error && <p className="mt-2 flex items-center gap-1 text-xs font-bold text-red-600"><XCircle className="size-4"/>{error}</p>}</div><Button type="button" variant="secondary" disabled={busy} onClick={() => input.current?.click()}><UploadCloud className="size-4"/>{busy ? `جارٍ الرفع ${progress ?? 0}%` : "اختيار ملف"}</Button></div></div>;
}
