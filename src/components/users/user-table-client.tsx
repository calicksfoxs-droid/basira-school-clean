...
import { Check, Copy, KeyRound, RotateCcw, ShieldAlert, X } from "lucide-react";
import {
  disableUserAction,
  reactivateUserWithRevealAction,
  resetAccessCodeWithRevealAction,
...
const initialCredentialState: ResetAccessCodeRevealState = { ok: true, data: undefined as never };
...
function CredentialActionButton({
  ...
  const serverAction = mode === "reactivate" ? reactivateUserWithRevealAction : resetAccessCodeWithRevealAction;
  const [state, action, pending] = useActionState(serverAction, initialCredentialState);
  ...
  const label = mode === "reactivate" ? "إعادة التفعيل" : "إعادة الرمز";
  const pendingLabel = mode === "reactivate" ? "جارٍ التفعيل…" : "جارٍ الإصدار…";
  ...
  return <>
    <form action={action}>
      ...
      <Button variant="secondary" size="sm" disabled={pending}>
        {mode === "reactivate" && <RotateCcw className="size-4"/>}
    ...
    {shouldReveal && <dialog ref={dialogRef} onCancel={(event) => { event.preventDefault(); closeReveal(); }} className="m-auto w-[min(92vw,560px)] rounded-[28px] border border-[var(--border)] bg-[var(--surface)] p-0 text-[var(--text)] shadow-2xl backdrop:bg-[#170b35]/70 backdrop:backdrop-blur-sm">
      <div className="p-6 sm:p-8" dir="rtl">
        ...
        <h2 className="font-heading mt-5 text-2xl font-bold">{mode === "reactivate" ? "تمت إعادة التفعيل" : "رمز دخول جديد"} لـ {revealed?.displayName}</h2>
        <p className="mt-2 text-sm leading-7 text-[var(--muted)]">انسخ الرمز الآن. أي رمز سابق أصبح غير صالح.</p>
...
export function UserTableClient({ users, returnTo, canManageCredentials }: { users: UserRecord[]; returnTo: string; canManageCredentials: boolean }) {
  return <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] text-[var(--text)] card-shadow">
    <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-right text-sm"><thead className="bg-[var(--soft)] text-xs text-[var(--muted)]"><tr><th className="p-4">الاسم</th><th className="p-4">الدور</th><th className="p-4">الحالة</th><th className="p-4">تاريخ الإنشاء</th><th className="p-4">إجراءات</th></tr></thead><tbody className="divide-y divide-[var(--border)]">{users.map((user) => {
      ...
      return <tr key={user.id}><td className="p-4 font-bold text-[var(--text)]">{user.displayName}</td><td className="p-4 text-[var(--text)]">{roleLabel(user.role)}</td><td className="p-4"><Badge tone={user.status === "active" ? "success" : "danger"}>{user.status === "active" ? "نشط" : "معطل"}</Badge></td><td className="p-4 text-[var(--muted)]">{formatDate(user.createdAt)}</td><td className="p-4"><div className="flex flex-wrap gap-2">{showResetButton && <CredentialActionButton userId={user.id} returnTo={returnTo} mode="reset"/>}{showReactivateButton && <CredentialActionButton userId={user.id} returnTo={returnTo} mode="reactivate"/>}{showDisableButton && <form action={disableUserAction}><input type="hidden" name="userId" value={user.id}/><input type="hidden" name="returnTo" value={returnTo}/><Button variant="danger" size="sm">تعطيل</Button></form>}</div></td></tr>;
    })}</tbody></table></div>
  </div>;
}
