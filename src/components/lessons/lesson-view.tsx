...
function ContentBlock({ identity, lessonId, lessonPartId, assets, quiz }: { identity: Identity; lessonId: string; lessonPartId?: string; assets: Asset[]; quiz?: Quiz }) {
  ...
  return <div className="grid gap-5">
    ...
    {editable && <details className="rounded-[22px] border border-[var(--border)] bg-[var(--surface)] p-5"><summary className="focus-ring cursor-pointer list-none font-heading text-lg font-bold text-[var(--brand)]">أدوات رفع محتوى الدرس</summary><div className="mt-5 grid gap-4 lg:grid-cols-2"><UploadPanel lessonId={lessonId} lessonPartId={lessonPartId} kind="video"/><UploadPanel lessonId={lessonId} lessonPartId={lessonPartId} kind="handout"/></div><div className="mt-4 rounded-2xl border border-[var(--border)] p-4"><h3 className="font-black">الاختبار</h3><p className="mt-1 text-xs text-[var(--muted)]">اختبار واحد في هذا المكان.</p><div className="mt-4">{quiz ? <LinkButton href={`/app/teacher/quizzes/${quiz.id}/edit`} variant="secondary">فتح الاختبار</LinkButton> : <LinkButton href={`/app/teacher/quizzes/new?lessonId=${encodeURIComponent(lessonId)}${lessonPartId ? `&lessonPartId=${encodeURIComponent(lessonPartId)}` : ""}`}>إنشاء اختبار</LinkButton>}</div></div></details>}
  </div>;
}
