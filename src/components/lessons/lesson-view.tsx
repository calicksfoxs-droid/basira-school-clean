...
import { Download, FileText, Layers3, PlayCircle, BookOpen } from "lucide-react";
...
function ContentBlock({ identity, lessonId, lessonPartId, assets, quiz, cover }: { cover: string; identity: Identity; lessonId: string; lessonPartId?: string; assets: Asset[]; quiz?: Quiz }) {
  ...
  return <div className="grid gap-5">
    ...
    <section className="rounded-[22px] border border-[var(--border)] bg-[var(--surface)] p-5"><h3 className="font-heading text-lg font-bold">الاختبار</h3>{supportedQuiz ? <><p className="mt-1 text-sm text-[var(--muted)]">{supportedQuiz.title}</p><div className="mt-4"><LinkButton href={identity.role === "teacher" ? `/app/teacher/quizzes/${supportedQuiz.id}/edit` : identity.role === "student" ? `/app/student/quizzes/${supportedQuiz.id}` : `/app/admin/lessons/${lessonId}`}>{identity.role === "student" ? "بدء الاختبار" : "فتح الاختبار"}</LinkButton></div></> : legacyQuiz ? <EmptyState compact title="اختبار قديم غير متاح في Core 1.0" description="الاختبارات المقالية والتصحيح اليدوي خارج السطح المدعوم الحالي."/> : <EmptyState compact title="لا يوجد اختبار بعد"/>}</section>
