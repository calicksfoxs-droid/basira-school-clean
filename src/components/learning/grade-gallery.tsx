...
export function GradeGallery({ identity, grades, subjects }: { identity: Identity; grades: CurriculumGrade[]; subjects: LearningSubject[] }) {
  ...
  return <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{grades.map((grade, index) => {
    ...
    return <Link key={grade.id} href={`/app/${identity.role}/grades/${grade.id}`} className="focus-ring grade-card group min-h-48 overflow-hidden rounded-[24px] border border-[var(--border)] bg-[var(--surface)] p-6 transition hover:-translate-y-1 hover:shadow-[0_18px_44px_rgba(23,32,51,.10)]">
