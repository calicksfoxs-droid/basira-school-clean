...
export function CurriculumSections({ identity, details }: { identity: Identity; details: LearningSubjectDetails }) {
  ...
  return <section className="curriculum-workspace grid min-w-0 gap-8">
    ...
    {SEGMENTS.map((segment) => {
      ...
      return <section key={segment.id} className="grid gap-4" aria-labelledby={`segment-${segment.id}`}>
        ...
        <div className="flex min-w-0 snap-x gap-4 overflow-x-auto pb-3 [scrollbar-width:thin]">
          {units.map((unit, unitIndex) => {
            ...
            return <article key={unit.id} className="w-[min(86vw,360px)] shrink-0 snap-start overflow-hidden rounded-[24px] border border-[var(--border)] bg-[var(--surface)] shadow-sm">
              ...
              <div className="p-4"><div className="flex items-start justify-between gap-3"><div><h4 className="font-heading text-lg font-bold">{unit.title}</h4><span className="mt-1 inline-block text-xs font-bold text-[var(--accent)]">{unit.status === "published" ? "وحدة منشورة" : "مسودة الوحدة"}</span><p className="mt-1 line-clamp-2 text-xs leading-5 text-[var(--muted)]">{unit.description || "وحدة تعليمية مرتبة"}</p></div><span className="shrink-0 rounded-full bg-[var(--soft)] px-2.5 py-1 text-[11px] font-bold">{lessons.length} دروس</span></div>
                <div className="mt-4 grid gap-2">{lessons.length ? lessons.map((lesson, lessonIndex) => <div key={lesson.id} className="flex min-h-14 items-center justify-between gap-2 rounded-2xl border border-[var(--border)] px-3 py-2">
                  <Link href={editable ? `/app/teacher/lessons/${lesson.id}/edit` : `/app/${identity.role}/lessons/${lesson.id}`} className="focus-ring flex min-w-0 flex-1 items-center gap-3 rounded-xl"><span className="grid size-8 shrink-0 place-items-center rounded-xl bg-[var(--soft)] text-xs font-black text-[var(--brand)]">{lessonIndex + 1}</span><span className="min-w-0"><strong className="block truncate text-sm">{lesson.title}</strong><small className="text-[11px] text-[var(--muted)]">{lesson.status === "published" ? "متاح" : "قيد الإعداد"}</small></span></Link>
                  {editable ? <span className="flex items-center"><ActionForm action={removeLearningLessonAction}><input type="hidden" name="lessonId" value={lesson.id}/><ConfirmSubmit name={lesson.title} label="إزالة"/></ActionForm>{lesson.status !== "published" && <ActionForm action={publishLearningLessonAction}><input type="hidden" name="lessonId" value={lesson.id}/><button className="focus-ring rounded-xl px-2 py-2 text-[11px] font-black text-[var(--brand)] hover:bg-[var(--soft)]">نشر</button></ActionForm>}</span> : <ChevronLeft className="size-4 text-[var(--brand)]"/>}
                </div>) : <EmptyState title="لا توجد دروس بعد"/>}</div>
                {editable && <ActionForm action={createLearningLessonAction} className="mt-3 flex flex-wrap items-end gap-2 border-t border-[var(--border)] pt-3"><input type="hidden" name="unitId" value={unit.id}/><input type="hidden" name="structureMode" value="direct"/><label className="grid min-w-0 flex-1 gap-1 text-xs font-bold">اسم الدرس<Input name="title" required placeholder="اسم الدرس" className="min-w-0"/></label><Button size="sm" aria-label="إضافة درس"><Plus className="size-4"/></Button></ActionForm>}
          ...
          {editable && <details className="w-[min(86vw,320px)] shrink-0 snap-start rounded-[24px] border border-dashed border-[var(--border)] bg-[var(--canvas)] p-5 open:bg-[var(--surface)]">
            ...
            <ActionForm action={createLearningUnitAction} className="mt-4 grid gap-3"><input type="hidden" name="subjectId" value={details.subject.id}/><input type="hidden" name="termSegment" value={segment.id}/><label className="grid gap-1 text-xs font-bold">اسم الوحدة<Input name="title" required placeholder="اسم الوحدة"/></label><label className="grid gap-1 text-xs font-bold">عدد الدروس المبدئي<Input name="lessonCount" type="number" min="0" max="40" defaultValue="0"/></label><Button><BookOpen className="size-4"/> إنشاء الهيكل</Button></ActionForm>
          </details>}
        </div>
      </section>;
    })}
