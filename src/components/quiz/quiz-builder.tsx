...
import { CheckCircle2, CirclePlus, GripVertical, Trash2 } from "lucide-react";
...
type DraftQuestion = { id: string; type: DraftQuestionType; prompt: string; points: number; options: Array<{ text: string; isCorrect: boolean }>; correctBoolean?: boolean };
...
export function QuizBuilder({ lessonId, lessonPartId }: { lessonId?: string; lessonPartId?: string }) {
  ...
  return <div className="grid gap-5"><div className="grid gap-4 sm:grid-cols-2"><Field label="عنوان الاختبار"><Input value={title} onChange={(e) => setTitle(e.target.value)} required/></Field><Field label="تعليمات قصيرة"><Input value={instructions} onChange={(e) => setInstructions(e.target.value)}/></Field></div>{questions.map((question, index) => <Card key={question.id} className="relative"><div className="mb-5 flex items-center justify-between"><div className="flex items-center gap-2"><GripVertical className="size-4 text-slate-400"/><strong>السؤال {index + 1}</strong></div><Button variant="ghost" size="sm" disabled={questions.length === 1} onClick={() => setQuestions((items) => items.filter((item) => item.id !== question.id))}><Trash2 className="size-4"/> حذف</Button></div><div className="grid gap-4"><div className="grid gap-4 sm:grid-cols-[200px_1fr_120px]"><Field label="النوع"><Select value={question.type} onChange={(e) => { const nextType = e.target.value as DraftQuestionType; update(question.id, { type: nextType, options: nextType === "mcq" ? question.options.length ? question.options : [{ text: "", isCorrect: true }, { text: "", isCorrect: false }] : [], correctBoolean: nextType === "true_false" ? (question.correctBoolean ?? true) : question.correctBoolean }); }}><option value="mcq">اختياري</option><option value="true_false">صح / خطأ</option></Select></Field><Field label="نص السؤال"><Textarea className="min-h-20" value={question.prompt} onChange={(e) => update(question.id, { prompt: e.target.value })}/></Field><Field label="الدرجة"><Input type="number" min={1} max={100} value={question.points} onChange={(e) => update(question.id, { points: Number(e.target.value) })}/></Field></div>{question.type === "mcq" && <div className="grid gap-3">{question.options.map((option, optionIndex) => <div key={optionIndex} className={`grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-2xl border p-3 ${option.isCorrect ? "border-[var(--accent)] bg-[var(--soft)]" : "border-[var(--border)]"}`}><input aria-label={`الخيار ${optionIndex + 1} هو الإجابة الصحيحة`} className="size-5" type="radio" name={`correct_${question.id}`} checked={option.isCorrect} onChange={() => update(question.id, { options: question.options.map((item, current) => ({ ...item, isCorrect: current === optionIndex })) })}/><label className="grid min-w-0 gap-2 text-xs font-bold"><span className="flex items-center gap-1">الخيار {optionIndex + 1}{option.isCorrect && <><CheckCircle2 className="size-4"/> الإجابة الصحيحة</>}</span><Input value={option.text} onChange={(e) => update(question.id, { options: question.options.map((item, current) => current === optionIndex ? { ...item, text: e.target.value } : item) })}/></label><Button variant="ghost" size="sm" disabled={question.options.length <= 2} onClick={() => update(question.id, { options: question.options.filter((_, current) => current !== optionIndex).map((item, current) => ({ ...item, isCorrect: option.isCorrect ? current === 0 : item.isCorrect })) })}>حذف</Button></div>)}<Button variant="secondary" size="sm" onClick={() => update(question.id, { options: [...question.options, { text: "", isCorrect: false }] })}><CirclePlus className="size-4"/> إضافة خيار</Button></div>}{question.type === "true_false" && <Field label="الإجابة الصحيحة"><Select value={String(question.correctBoolean ?? true)} onChange={(e) => update(question.id, { correctBoolean: e.target.value === "true" })}><option value="true">صح</option><option value="false">خطأ</option></Select></Field>}</div></Card>)}<div className="flex flex-wrap items-center gap-3"><Button variant="secondary" onClick={() => setQuestions((items) => [...items, makeQuestion()])}><CirclePlus className="size-4"/> سؤال جديد</Button><Button disabled={pending} onClick={submit}>{pending ? "جارٍ الحفظ..." : "حفظ ونشر الاختبار"}</Button>{error && <p className="text-sm font-bold text-red-600">{error}</p>}</div></div>;
}
