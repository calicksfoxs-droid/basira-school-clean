...
test("يخفي الاختبار Legacy اليدوي من مسار الطالب", async ({ page }) => {
  await login(page, "student");
  await page.goto(`/app/student/lessons/${seededLessonId}`);
  await expect(page.getByText("اختبار قديم غير متاح في Core 1.0", { exact: true })).toBeVisible();
  await expect(page.getByRole("link", { name: "بدء الاختبار" })).toHaveCount(0);

  await page.goto(`/app/student/quizzes/${seededQuizId}`);
  await expect(page).toHaveURL(new RegExp(`/app/student/lessons/${seededLessonId}import { expect, test, type Page } from "@playwright/test";
import { rm } from "node:fs/promises";
import path from "node:path";
